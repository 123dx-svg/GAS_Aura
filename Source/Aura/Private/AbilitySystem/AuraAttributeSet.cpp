...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	//这里添加属性做了简化处理 高级委托机制不再需要创建单个返回值委托了
...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<class FLifetimeProperty>& OutLifetimeProps) const
{
	...
	//此处用的REPNOTIFY_Always而非Change是想如果变回原来的值，客户端也有回应
	...
	FEffectProperties& Props) const
{
	...
	//Source是施加效果者  Target是受影响的对象也是AS的拥有者
...
void UAuraAttributeSet::PostGameplayEffectExecute(const struct FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetHealthAttribute())
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("DX>>> Changed Health On %s, Health: %f"), *Props.TargetAvatarActor->GetName(), GetHealth());
	}
